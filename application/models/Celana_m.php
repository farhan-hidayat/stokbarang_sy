...
class Celana_m extends CI_Model {

    var $column_order = array(null, 'kode_barang', 'nama_barang', 'nama_suplier', 'harga', 'jenis', 's' , 'm' , 'l' , 'xl' , 'xxl' , 'xxxl' , 'xxxxl', 'xxxxxl'); //set column field database for datatable orderable
    var $column_search = array('kode_barang', 'nama_barang', 'harga', 'nama_suplier', 'jenis2'); //set column field database for datatable searchable
    var $order = array('id_barang' => 'asc'); // default order
 
    private function _get_datatables_query() {
        $this->db->from('barang');
        $this->db->join('suplier', 'suplier.id_suplier = barang.id_suplier');
        $this->db->where('kategori = "Celana" AND jenis2 = "Dewasa"');
        $i = 0;
    ...
    private function _get_datatables_query2() {
        $this->db->from('barang');
        $this->db->join('suplier', 'suplier.id_suplier = barang.id_suplier');
        $this->db->where('kategori = "Celana" AND jenis2 = "Anak"');
        $i = 0;
        foreach ($this->column_search as $barang) { // loop column
            if(@$_POST['search']['value']) { // if datatable send POST for search
                if($i===0) { // first loop
